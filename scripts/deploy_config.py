...
def main() -> int:
    ...
    out("  FLOTTA_GITHUB_TOKEN=<a fine-grained PAT>   <- optional, see below")
    out()
    out("  # What every box it creates needs in order to boot and answer.")
    out("  # Without these a created agent crash-loops on its first start.")
    out("  FLOTTA_BOX_PASSWORD=<the value just fly-auth wrote into .env>")
    out("  FLOTTA_MODEL=<e.g. z-ai/glm-5.2>")
    out("  FLOTTA_MODEL_BASE_URL=<e.g. https://openrouter.ai/api/v1>")
    out("  FLOTTA_API_KEY=<your provider key>")
    out("  FLOTTA_CONTROL_URL=https://<your-railway-app>.up.railway.app")
    out("  FLOTTA_DOMAIN=<your domain>")
    out()
    out("  The last two look redundant on the control plane and are not: it")
    out("  stamps them onto every box it creates, so the box knows where to")
    out("  fetch git credentials and what domain to sign commits under. It")
    out("  cannot infer its own public URL.")
    out()
    out("  FLOTTA_BOX_PASSWORD must be the SAME value the door has — the door")
    out("  logs into every box with it. A different one locks the door out.")
    out()
    out("  The three Fly variables are not optional. The control plane is the")
    out("  code that reaches the substrate: it creates, wakes, destroys and")
    out("  suspends machines. Without them it serves reads, fails every write,")
    out("  and the idle sweep fails silently — so the fleet never sleeps while")
    out("  /health keeps reporting a healthy loop.")
    out()
    out("  FLOTTA_GITHUB_TOKEN is what boxes trade their own token for when")
    out("  they clone or push. Not generated here: a PAT is minted on GitHub,")
    out("  scoped to the repositories you are willing to lose. Without it the")
    out("  credential endpoint answers 503 and boxes read public repos only.")
    out("  Flotta enforces which box may use which repository; GitHub does")
    out("  not, so scope the PAT rather than trusting the grant table alone.")
    out()
