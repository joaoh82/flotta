...
def _skill():
    path = SKILLS_DIR / "flotta" / "flotta-github-access" / "SKILL.md"
    text = path.read_text(encoding="utf-8")
    _, front, body = text.split("---", 2)
    return yaml.safe_load(front), body


def test_the_skill_is_where_hermes_looks_for_one():
    ...
    assert [p.parent.name for p in skill_dirs] == [front["name"]]
    category = skill_dirs[0].parent.parent.name
    demoted = {
    ...
    assert category not in demoted


def test_the_description_fits_in_the_prompt_and_names_the_command():
    ...
    assert "flotta-repos" in front["description"]


def test_the_skill_names_a_command_that_is_installed():
    ...
    assert scripts["flotta-repos"] == "flotta.box.repos:main"
    _, body = _skill()
    assert "flotta-repos" in body
