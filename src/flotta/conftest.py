...
def _hermetic_environment(monkeypatch):
    ...
    **No exceptions, which is why the suite's own Postgres switch is called
    `PYTEST_POSTGRES_URL` and not `FLOTTA_TEST_POSTGRES_URL`.** It configures
    the harness rather than the code under test, so it has to survive — and the
    first attempt exempted it here by name, which
    `test_the_suite_is_hermetic_against_a_developers_env` correctly refused: a
    rule with one carve-out is a rule that grows a second. Renaming it out of
    the namespace makes the variable *comply* instead of being excused, and
    leaves the guard able to say something absolute.
    """
...
def pytest_sessionstart(session):
    ...
    Read from the real environment, and unaffected by the per-test stripping
    for two reasons: this runs at session start, and the variable is
    `PYTEST_POSTGRES_URL` rather than `FLOTTA_*` precisely because it configures
    ...
    raise pytest.UsageError(
        "PYTEST_POSTGRES_URL is unset on CI, so the Postgres transaction "
